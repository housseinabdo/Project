...
import classes.*;
public class AleatoryFailureMain extends JFrame {
	...
	private JCheckBox chckbxFixeTime;
	private JTextArea lblMassReleased1;
	private JLabel lblMaxMass;
	private JLabel lblPanneNoResolut;
	private static int PaneNoResolut = 0;
	private static double fixe_tbreak, fixe_TRmin, fixeTR_top, fixeTR_max;
	...
	static HashMap<Double, Double> ResponceTimeAndDegree; // key
	private JTextField textCut;
	private JLabel Cut;
	private JCheckBox chcTimeResponse;
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				try {
					AleatoryFailureMain frame = new AleatoryFailureMain();
	...
	class MyThread extends Thread {
		...
		public void run() {
			...
			ResponceTimeAndDegree = new HashMap<Double, Double>();
			TriangularFuzzyNumber TR_rectangle = null, DEC = null, TrianM = null, TrianMC = null, TrianTR = null, TrianX = null, TrianOpen = null, TrianOut = null;
			try {
			...
			if (Gaussian.isSelected())
				monteCarlo = 2;
			double MTBF = 10000, u = Math.random(), t_break, TR_min, TR_top, TR_max, t_reponce;
			t_break = (-MTBF) * Math.log((1 - u));
			while (t_break > 900) {
				t_break -= 900;
			}
			u = Math.random();
			TR_min = (-DEC.getA()) * (Math.log(1 - u));
			TR_top = (-DEC.getB()) * (Math.log(1 - u));
			TR_max = (-DEC.getC()) * (Math.log(1 - u));

			if (chckbxFixeTime.isSelected()) {
				t_break = fixe_tbreak;
			}
			if (chcTimeResponse.isSelected()) {
				TR_min = fixe_TRmin;
				TR_top = fixeTR_top;
				TR_max = fixeTR_max;
			}
			...
			try {
				TR_rectangle = new TriangularFuzzyNumber(TR_min, TR_top, TR_max);
			} catch (FuzzyOntologyException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			for (int counter = 0; counter < Count; counter++) {
				...
				t_reponce = ran.simule(TR_min, TR_max, monteCarlo);

				double deg_reponce = TR_rectangle
						.getMembershipDegree(t_reponce);

				ResponceTimeAndDegree.put(t_reponce, deg_reponce);

				FirstOrderDifferentialEquations ode = new CircleODE(Tbreakk,
						KK, FFC, t_break, t_reponce);
				...
				if (somme > 0) {
					PaneNoResolut++;
					lblPanneNoResolut.setText("" + PaneNoResolut);
				}
			...
			// Drow repoce a la panne
			final DrowChart demo1 = new DrowChart("Responce",
					ResponceTimeAndDegree, "Temp_Reponce", "membershipDegree");
			demo1.pack();
	...
	public AleatoryFailureMain() {
		...
		JLabel lbl = new JLabel("Panne no resolut");

		lblPanneNoResolut = new JLabel("0");
		lblPanneNoResolut.setHorizontalAlignment(SwingConstants.RIGHT);
						...
						gl_panel.createSequentialGroup()
								...
								.addGroup(
										gl_panel.createParallelGroup(
												...
												.addComponent(lbl)
												.addComponent(lblPanneNoResolut)
												.addComponent(
														lblCounter,
														GroupLayout.PREFERRED_SIZE,
														14,
		...
		chckbxFixeTime = new JCheckBox("FixedFail");
		chckbxFixeTime.setBounds(382, 61, 86, 23);
		panel_3.add(chckbxFixeTime);

		chcTimeResponse = new JCheckBox("FixedResp");
		chcTimeResponse.setBounds(281, 62, 97, 23);
		panel_3.add(chcTimeResponse);
